...
def _ensure_break(prev_end, next_start, min_minutes: int) -> bool:
    delta = (timezone.datetime.combine(timezone.now().date(), next_start) -
...
def generate_class_timetable(timetable: models.Timetable) -> Dict:
    ...
    def can_place(course: models.Course, instructor_id: int, room_obj: models.Room, 
                  slot: models.Slot, section: str, is_practical: bool = False) -> bool:
        ...
        if not any(day == slot.day_of_week and a_start <= slot.start_time and a_end >= slot.end_time 
                  for day, a_start, a_end in prof_avails):
            return False
        
        # Room availability
        room_avails = room_to_avails.get(room_obj.id, [])
        if not any(day == slot.day_of_week and r_start <= slot.start_time and r_end >= slot.end_time 
        ...
        for m in mess_hours:
            if m.day_of_week == slot.day_of_week and _time_overlap(m.start_time, m.end_time, slot.start_time, slot.end_time):
                return False
        ...
        for course in courses:
            ...
            for slot in slots:
                ...
                for room_obj in room_candidates:
                    if not can_place(course, primary_instructor, room_obj, slot, section, practical_needed > 0):
                        continue
                    ...
                    if lecture_needed > 0 and slot.day_of_week not in course_day_booked[(course.id, section)]:
                        ...
                        prof_booked[primary_instructor].append((slot.day_of_week, slot.start_time, slot.end_time))
...
def generate_exam_schedule() -> Dict:
    ...
    rooms = list(models.Room.objects.filter(room_type__in=[models.RoomType.HALL, models.RoomType.CLASSROOM]).order_by("-capacity"))
...
def balance_invigilation(exam: models.Exam) -> Dict:
    ...
    for room_id in rooms:
        ...
        models.InvigilationDuty.objects.create(
            exam=exam,
            professor=professor,
            room_id=room_id
        )
...
def check_timetable_conflicts(timetable_id: int) -> Dict:
    ...
    for instructor_id, instructor_sessions_list in instructor_sessions.items():
        for i, session1 in enumerate(instructor_sessions_list):
            for session2 in instructor_sessions_list[i+1:]:
                if (session1.slot.day_of_week == session2.slot.day_of_week and
                    _time_overlap(session1.slot.start_time, session1.slot.end_time,
                                session2.slot.start_time, session2.slot.end_time)):
                    conflicts.append({
    ...
    for instructor_id, instructor_sessions_list in instructor_sessions.items():
        ...
        for i in range(len(instructor_sessions_list) - 1):
            ...
            if (session1.slot.day_of_week == session2.slot.day_of_week and
                not _ensure_break(session1.slot.end_time, session2.slot.start_time, 15)):
                conflicts.append({
